...
Specifically, the "kubo" package in this project can glue together the full Green's function for an orbitronic bilayer system with a plane interface between two metals that breaks the translation invariance of the system perpendicular to the interface. The Kubo integral is split up into the Fermi-surface and the Fermi-sea contributions according to the Streda decomposition formula, which holds true in the DC limit.
